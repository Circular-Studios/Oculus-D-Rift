...
enum LogMaskConstants
{
    ...
    LogMask_Debug   = 0x200,
    LogMask_None    = 0,
    LogMask_All     = OVR.LogMaskConstants.LogMask_Regular|OVR.LogMaskConstants.LogMask_Debug
};
...
{    
    // General Logging
    Log_Text        = OVR.LogMaskConstants.LogMask_Regular | 0,    
    Log_Error       = OVR.LogMaskConstants.LogMask_Regular | 1, // "Error: %s\n".
    
    // Debug-only messages (not generated in release build)
    Log_DebugText   = OVR.LogMaskConstants.LogMask_Debug | 0,
    Log_Debug       = OVR.LogMaskConstants.LogMask_Debug | 1,   // "Debug: %s\n".
    Log_Assert      = OVR.LogMaskConstants.LogMask_Debug | 2,   // "Assert: %s\n".
};


// LOG_VAARG_ATTRIBUTE macro, enforces printf-style fromatting for message types
static if(__GNUC__) {
 auto OVR_LOG_VAARG_ATTRIBUTE(  ARG1,  ARG2 )(ARG1 a, ARG2 b) { return __attribute__((format (printf, a, b))); }
} else {
 void OVR_LOG_VAARG_ATTRIBUTE(  ARG1,  ARG2 )(ARG1 a, ARG2 b) { {} }
}
...
//    friend class System;
public: 
    this(uint logMask = OVR.LogMaskConstants.LogMask_Debug) { LoggingMask = (logMask); }
    ~Log();

    // Log formating buffer size used by default LogMessageVarg. Longer strings are truncated.
    enum { MaxLogBufferMessageSize = 2048 };

    uint        GetLoggingMask() const            { return LoggingMask; }
    void            SetLoggingMask(uint logMask)  { LoggingMask = logMask; }

    // This virtual function receives all the messages,
    // developers should override this function in order to do custom logging
    void    LogMessageVarg(LogMessageType messageType, const(char) * fmt, va_list argList);
    ...
    static void     FormatLog(char * buffer, uint bufferSize, LogMessageType messageType,
    ...
    {
        return (messageType & OVR.LogMaskConstants.LogMask_Debug) != 0;
    }
    ...
    static Log*     ConfigureDefaultLog(uint logMask = OVR.LogMaskConstants.LogMask_Debug)
    {
...
static if(OVR_BUILD_DEBUG) {
    ...
    // as in: OVR_LOG_DEBUG(("Value %d", 2)).
     void OVR_DEBUG_LOG(  ARG1 )(ARG1 args) {        do { OVR.LogDebug( args ); } while(0); }
     void OVR_DEBUG_LOG_TEXT(  ARG1 )(ARG1 args) {   do { OVR.LogDebugText( args ); } while(0); }

     void OVR_ASSERT_LOG(  ARG1,  ARG2 )(ARG1 c, ARG2 args) {    do { if (!c) { OVR.LogAssert( args ); OVR_DEBUG_BREAK; } } while(0); }

} else {

    // If not in debug build, macros do nothing.
     void OVR_DEBUG_LOG(  ARG1 )(ARG1 args) {          (cast()0); }
     void OVR_DEBUG_LOG_TEXT(  ARG1 )(ARG1 args) {     (cast()0); }
